...
pub fn evaluate_rule(rule: &PolicyRule, ctx: &EvaluationContext) -> Result<bool, CoreError> {
    match rule {
        ...
        PolicyRule::Not(inner) => Ok(!evaluate_rule(inner, ctx)?),
...
mod rule_evaluator_tests {
    ...
    fn test_eval_not_inverts() {
        ...
        let rule = PolicyRule::Not(Box::new(PolicyRule::ChainMatch {
            chain: "bitcoin".into(),
        }));
        assert!(evaluate_rule(&rule, &base_ctx()).unwrap());

        // ChainMatch("ethereum") is true, so Not should be false
        let rule2 = PolicyRule::Not(Box::new(PolicyRule::ChainMatch {
            chain: "ethereum".into(),
        }));
    ...
    fn test_eval_velocity_under_limit() {
        ...
        let rule = PolicyRule::VelocityLimit {
            max_amount: 2000,
            window_secs: 86400,
        };
        assert!(evaluate_rule(&rule, &base_ctx()).unwrap());
